...
def main():
    ...
    WORKING_DIR = f"/mnt/ephemeral/migration_test"
    ...
    run_ssh_command(src_srv_ssh_client, f"yes | sudo mkfs -t ext4 /dev/nvme2n1")
    run_ssh_command(src_srv_ssh_client, f"sudo mount /dev/nvme2n1 {EBS_DIR}")
    ...
    while checkpoint_num <= PRE_DUMP_NUM:
        ...
        stdout, stderr = run_ssh_command(src_srv_ssh_client, f"rsync -av --whole-file --no-compress --inplace --links {WORKING_DIR} {EBS_DIR}")
        print(stdout, stderr)
    ...
    run_ssh_command(dst_srv_ssh_client, f"sudo mount /dev/nvme2n1 {EBS_DIR}")
    run_ssh_command(src_srv_ssh_client, f"sudo chown {SSH_USER_NAME}:{SSH_USER_NAME} -R {EBS_DIR}")
    ebs_volume_attach_duration = time.time() - ebs_volume_attach_start_time

    # Copy to instance store
    print("Copying dump datas to instance store...")
    copy_start_time = time.time()
    stdout, stderr = run_ssh_command(dst_srv_ssh_client, f"rsync -av --whole-file --no-compress --inplace --links {EBS_DIR}/migration_test/* {WORKING_DIR}")
    ...
    run_ssh_background_command(dst_srv_ssh_client, f"sudo criu lazy-pages --images-dir {WORKING_DIR}/{checkpoint_num} --page-server --address {src_srv_ip} --port {PAGE_SERVER_PORT} &")
    page_server_duration = time.time() - page_server_start_time

    # CRIU restore
    print("Starting CRIU restore...")
    restore_start_time = time.time()
    stdout, stderr = run_ssh_command(dst_srv_ssh_client, f"sudo criu restore -D {WORKING_DIR}/{checkpoint_num} --shell-job --lazy-pages")
    ...
    print(f" - Dump File Copy time: {copy_duration:.2f} seconds")
