...
def main():
    ...
    dst_srv_id = AZ_C_INSTANCES_IDs[0]
    ...
    # Snapshot EBS Volume
    print("Creating snapshot of EBS Volume...")
    ...
    print("Creating EBS Volume from Snapshot...")
    volume_create_start_time = time.time()
    new_volume = ec2.create_volume(SnapshotId=snapshot_id, AvailabilityZone=f"{REGION}c")
    new_volume_id = new_volume['VolumeId']
    while True:
        volume = ec2.describe_volumes(VolumeIds=[new_volume_id])['Volumes'][0]
        state = volume['State']
        if state == 'available':
            break
    volume_create_duration = time.time() - volume_create_start_time

    # Attach EBS Volume on Dst Server
    print("Attaching EBS Volume on Dst Server...")
    ebs_volume_attach_start_time = time.time()    
    attach_volume(new_volume_id, dst_srv_id)
    ...
    # Remove snapshot and volumes
    print("Cleaning up...")
    ec2.delete_snapshot(SnapshotId=snapshot_id)
    detach_volume(new_volume_id)
    ...
    print(f" - EBS Snapshot create time: {snapshot_duration:.2f} seconds")
    print(f" - EBS Volume migration time: {volume_create_duration:.2f} seconds")
