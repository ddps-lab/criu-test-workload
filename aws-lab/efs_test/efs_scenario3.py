...
def write_performance_test_on_host(host, username):
    commands = [
        'sudo fio --filename=/mnt/efs/testfile --direct=1 --rw=write --bs=1M --size=8G --numjobs=1 --name=writetest --iodepth=64'
    ]
    results = execute_ssh_command(host, username, commands)
    for cmd, out, err, status in results:
        if status != 0:
...
def main():
    ...
    print("Benchmarking write EFS on Host A (On AZ a)")
    write_performance_test_on_host(host_a_az_a_ip, username)

    print("Delete Test File on EFS")
    execute_ssh_command(host_a_az_a_ip, username, ['sudo rm /mnt/efs/testfile'])

    print("Benchmarking write EFS on Host A (On AZ c)")
    write_performance_test_on_host(host_a_az_c_ip, username)
